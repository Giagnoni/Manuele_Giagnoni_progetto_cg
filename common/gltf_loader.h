...
struct gltf_loader {
	...
	void visit_node(glm::mat4  currT, int i_node) {
		...
		const std::vector<double>& tv = model.nodes[i_node].translation;
		const std::vector<double>& rv = model.nodes[i_node].rotation;
		const std::vector<double>& sv = model.nodes[i_node].scale;
		...
		if (!m.empty())
			transform = glm::mat4(m[0], m[1], m[2], m[3],
				...
				m[8], m[9], m[10], m[11],
				m[12], m[13], m[14], m[15]);
		else if (!(tv.empty() && rv.empty() && sv.empty())) {
			if (!tv.empty() && tv.size() >= 3)
				T = glm::translate(glm::mat4(1.0), glm::vec3(tv[0], tv[1], tv[2]));
			if (!rv.empty() && rv.size() >= 4)
				R = glm::mat4_cast(glm::quat(rv[3], glm::vec3(rv[0], rv[1], rv[2])));
			if (!sv.empty() && sv.size() >= 3)
				S = glm::scale(glm::mat4(1.0), glm::vec3(sv[0], sv[1], sv[2]));
			transform = T * R * S;
		}
		...
		if (!model.nodes[i_node].children.empty()) {
			for (unsigned int ic = 0; ic < model.nodes[i_node].children.size(); ++ic)
				visit_node(currT * transform, model.nodes[i_node].children[ic]);
		}
	}
	...
	bool create_renderable(std::vector<renderable>& _renderable, box3& bbox) {
		...
		for (int i = 0; i < scene.nodes.size(); i++) {
			visit_node(glm::mat4(1.f), scene.nodes[i]);
		}
