...
std::vector<stick_object> alberi;
...
gltf_loader carica_auto, carica_albero;
std::vector<renderable> modelli_auto, modello_albero;
box3 box;
...
void draw_scene(shader sh) {
	...
	glVertex3f(0, 0, 0);
	glVertex3f(dirL.x, dirL.y, dirL.z);
	glEnd();
	...
	glCullFace(GL_BACK);
	if (sh.has_uniform("oggetto_mappato")) glUniform1i(sh["oggetto_mappato"], 2);
	float scala = 1.f / modello_albero[0].bbox.diagonal() * 10;
	for (int i = 0; i < alberi.size(); i++) {
		...
		for (int i = 0; i < modello_albero.size(); i++) {
			modello_albero[i].bind();
			stack.push();
			stack.mult(modello_albero[i].transform);

			glActiveTexture(GL_TEXTURE0);
			glBindTexture(GL_TEXTURE_2D, modello_albero[i].mater.base_color_texture);

			glUniformMatrix4fv(sh["model_matrix"], 1, GL_FALSE, &stack.m()[0][0]);
			glDrawElements(modello_albero[i]().mode, modello_albero[i]().count, modello_albero[i]().itype, 0);
			stack.pop();
		}
		stack.pop();
	}
...
int main() {
	...
	Lproj = glm::ortho(-2.f, 2.f, -2.f, 2.f, 0.f, 5.f);
	...
	glUniform1f(shader_program["uBias"], 0.003);
	glUniform1i(shader_program["uShadowMap"], 0);
	glUniform2i(shader_program["uShadowMapSize"], w, h);
